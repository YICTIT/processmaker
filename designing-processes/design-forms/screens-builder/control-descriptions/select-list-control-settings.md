...
15. [Save your ProcessMaker Screen](../save-a-screen.md#save-a-processmaker-screen), and then continue to [configure the Select List control's data source that gets the list of regions](select-list-control-settings.md#configure-the-select-list-controls-data-source-that-gets-the-list-of-regions).
...
1. [Log on](../../../../using-processmaker/log-in.md#log-in) to ProcessMaker.
2. Edit the ProcessMaker Screen for this example. Ensure that this ProcessMaker Screen [contains the first Select List control configured for this example](select-list-control-settings.md#configure-the-select-list-controls-data-source-that-gets-the-list-of-regions).
...
7. In the **Options Variable** setting, enter `regions` since the Watcher outputs to Request data the JSON array it receives from the ProcessMaker Data Connector that gets the list of regions for the selected country using that Request variable name. ![](../../../../.gitbook/assets/dependent-fields-example-options-variable-setting-select-list-control.png) 
