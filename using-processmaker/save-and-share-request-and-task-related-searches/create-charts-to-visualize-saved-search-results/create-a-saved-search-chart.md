...
5. From the **Chart Type** group, select one of the following chart types to visualize the results of this Saved Search:
   ...
   * **Count:** The count chart displays a single data metric on a canvas. See [chart example](overview-of-saved-search-charts.md#count). If you select the **Count** chart option, see [Create a Count Chart]() for its specific settings.
   * **List:** The list chart displays chart data in tabular format. See [chart example](overview-of-saved-search-charts.md#list-chart). If you select the **List** chart option, see [Create a Count Chart]() for its specific settings.
...
3. Click the **+Chart** button. The **Create Chart** screen displays the **General** tab. ![](../../../.gitbook/assets/general-tab-create-a-chart-saved-searches-package.png) 
4. In the **Name** setting, enter the name of the chart. This is a required setting.
5. From the **Chart Type** group, select the Count chart type to visualize the results of this Saved Search. The **count** chart displays a single data metric on a canvas. See [chart example](overview-of-saved-search-charts.md#count).
...
8. In the **Query** setting, optionally enter a ProcessMaker Query Language \(PMQL\) query from which to further filter the Saved Search's results. See the following topics for information how to use PMQL for Requests, Tasks, and ProcessMaker Collections:
   ...
   * [PMQL Syntax for Tasks](../../search-processmaker-data-using-pmql.md#pmql-syntax-for-tasks)
   * [PMQL Syntax for ProcessMaker Collections](../../search-processmaker-data-using-pmql.md#pmql-syntax-for-processmaker-collections)
9. Click the **Display** tab to configure the appearance and style of the Saved Search chart. The **Display** tab settings vary depending on the chart type selected from the **General** tab.
...
3. Click the **+Chart** button. The **Create Chart** screen displays the **General** tab. ![](../../../.gitbook/assets/general-tab-create-a-chart-saved-searches-package.png) 
4. In the **Name** setting, enter the name of the chart. This is a required setting.
5. From the **Chart Type** group, select one of the following chart types to visualize the results of this Saved Search. The **list** chart displays chart data in tabular format. See [chart example](overview-of-saved-search-charts.md#list-chart).

   **Bar \(Horizontal\)** is the default chart type.

6. Click the **Source** tab to configure from which Saved Search's results to visualize in the chart. ![](../../../.gitbook/assets/source-tab-create-chart-screen-saved-searches-package.png) 
7. Select the **Select Pivot Table** icon . If enabled ![](../../../.gitbook/assets/display-pivot-table-icon-create-a-list-chart-saved-searches-package.png)the following settings display:
...
9. Click the **Display** tab to configure the appearance and style of the Saved Search chart. The **Display** tab settings vary depending on the chart type selected from the **General** tab.
   ...
   ~~\_\_~~![](../../../.gitbook/assets/create-a-list-chart-display-pivot-icon-enabled-saved-searches-package.png) 

10. From the **Background Color** setting, select a color as the background for the count chart canvas.
11. If the **Select Pivot Table** icon is disabled ![](../../../.gitbook/assets/display-pivot-disabled-table-icon-create-a-list-chart-saved-searches-package.png) , the following settings display:

    1. Activate the **Display link button** icon to disable or enable if wish to display a link to the corresponding record at the end of each row.
    2. Activate the **Make rows clickable** icon to disable or enable if wish to select whether clicking anywhere within a row will link to the corresponding record.  

    ![](../../../.gitbook/assets/create-a-list-chart-display-pivot-icon-disabled-saved-searches-package.png) 
